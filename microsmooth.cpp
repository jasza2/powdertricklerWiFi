...
long normalization_value;

long* ms_init(uint8_t algo)
{
    if(algo & SMA) 
    {
	return (long *)calloc(SMA_LENGTH, sizeof(long));
    }
	...
	return (long *)calloc(SGA_LENGTH, sizeof(long));
    }
    else if(algo & KZA) 
    {
	return (long *)calloc(KZA_LENGTH, sizeof(long));
    }
    else if(algo & RDP) 
    {
	return (long *)calloc(RDP_LENGTH, sizeof(long));
    }
    else if(algo & KFA) 
    {
    }
...
int sma_filter(int current_value, long history_SMA[])
{  
    long sum=0; /*This constrains SMA_LENGTH*/
    long average=0;
    uint8_t i;
...
{ 
    static long cumulative_average=0; /*Limiting factor for k*/
    static long k=0; /*Limiting factor for runtime*/
...
{ 
    static long exponential_average=current_value;
...
int sga_filter(int current_value, long history_SGA[])
{ 
    long sum=0;
    long SGA_MID = SGA_LENGTH/2;
    uint8_t i;
...
void rdp_recur(int start_index, int end_index, long history_RDP[])
{
...
int rdp_filter(int current_value, long history_RDP[])
{ 
    int16_t a; 
    int8_t b;
    int16_t c;
...
int kza_filter(int current_value, long history_KZA[])
{ 
  uint32_t divisor=1;
...
/* Kalman Filter : TBA */
int kfa_filter(int current_value, long history_KFA[])
{
    return 0;
};
